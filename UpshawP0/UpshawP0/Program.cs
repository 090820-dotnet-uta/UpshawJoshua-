    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var DBcontext = new SGDB2Context())
            {
                ...
                {
                    MenuAscii();
                    firstMenuChoice = UserMenuChoice();
                    ...
                    do
                    {
                        ...
                        {
                            RetrieveOrderHistory(CurrentlyLoggedIn, DBcontext);
                        }
                        else if(secondMenuChoice == 2)
                        {
                            SearchForUser(DBcontext);
                        }
                        else if(secondMenuChoice == 3)
                        {
                            GetStoreInventory(DBcontext);
                        }
                        else if (secondMenuChoice == 4)
                        {
                            SetSelectedStore(CurrentlyLoggedIn, DBcontext);
                            DBcontext.SaveChanges();
                        }
                        else if (secondMenuChoice == 5)
                        {
                            PlaceOrders(CurrentlyLoggedIn, DBcontext);
                            DBcontext.SaveChanges();
                        }
...
";
        ...
        /// <summary>
        /// Retrieves a newly made customer's address
        /// </summary>
        /// <param name="dbContext"></param>
        /// <returns>CustomerAddress</returns>
        public static Customers NewCustomerAddress(SGDB2Context dbContext)
        {
            string City;
            string State;
            string Street;
            ...
            Customers newAddress = new Customers();
            ...
            do
            {
                ...
                isInt = int.TryParse(City, out tester);
            } while (isInt || City == null);
            do
            {
                Console.WriteLine("\tPlease enter your state (abbreviated):");
                State = Console.ReadLine();
                ...
                isInt = int.TryParse(City, out tester);
            } while (State == null || State.Length > 2 || isInt);
            do
            {
                Console.WriteLine("\tPlease enter your street address:");
                Street = Console.ReadLine();
            ...
            //TODO ADD CHECKS TO SEE IF THIS IS ALREADY A REGISTERED ADDRESS AND ASSIGN THE CORRESPONDING ADDRESS ID TO THE CUSTOMER IF SO
            newAddress.City = City;
            newAddress.State = State;
            newAddress.Street = Street;
            newAddress.Zip = Zip;
            newAddress.AptNum = Apartment;
            return newAddress;

        }//Discarded function unless I re-implement customer address table 
        ...
        /// </summary>
        /// <param name="dbContext"></param>
        public static void SearchForUser(SGDB2Context dbContext)
        {
        ...
        public static void GetStoreInventory(SGDB2Context dbContext)
        {
            ...
            do //Do while loop to validate the user's choice of store
            {
                ...
                var inventorySelect = (from Inventory in dbContext.Inventory
                                       ...
                                       select new { Inventory.StoreInventory, 
                                                    Inventory.ItemInInventory, 
                                                    Inventory.ProductCurrentQuantity }).ToList();
                //Now we get all the information about all our products (That arent included in a bundle)
                var productInfo = (from products in dbContext.Products
                                   where products.IsInBundle == false
                                   select products).ToList();
        ...
        public static void SetSelectedStore(Customers LoggedInCustomer, SGDB2Context dbContext)
        {
            int menuChoice;
            int storeChoice = -1;
            string tester;
        ...
        public static decimal GetOrderTotal(decimal itemTotals)
        {
        ...
        //Function for outputting the inventory for a given store 

        //Function for the actual shopping  (goes until the 'Checkout' option is selected)
